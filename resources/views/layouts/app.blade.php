...
<head>
    ...
    <style>
        ...
        .header-sub { font-size: 11px; color: #aaa; margin: 0; }
        .sidebar { width: 210px; min-height: calc(100vh - 52px); background: #fff; border-right: 1px solid #f0f0f0; padding: 16px 0; flex-shrink: 0; }
        ...
        .nav-item { padding: 9px 14px; font-size: 13px; color: #555; border-left: 2px solid transparent; display: flex; align-items: center; text-decoration: none; }
        .nav-item:hover { background: #f4f4f9; color: #333; }
        .nav-item.active { background: #e8f5e9; color: #2e7d32; border-left-color: #4CAF50; font-weight: 600; }
...
<div class="d-flex">
    <div class="sidebar d-flex flex-column">
        <div class="avatar-area">
            <img src="{{ asset('storage/foto/' . Auth::user()->foto) }}" class="avatar-circle" alt="Foto">
    ...
    <div class="main-content">
        @if(session('sukses'))
        <div class="alert alert-success alert-dismissible fade show">{{ session('sukses') }}<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
        @endif
        @if(session('gagal'))
        <div class="alert alert-danger alert-dismissible fade show">{{ session('gagal') }}<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
        @endif
        @yield('content')
    </div>
</div>
